  ...
  switch (method) {
    case 'GET':
      ...
      const { data: playlist_song } = await supabase
        ...
        .order('id');
      const { data: playlist } = await supabase.from('playlist_user').select(`*`).eq('id', query.id).order('id');
    ...
    case 'POST':
      if (!body.id_song) {
        ...
        const { error } = await supabase.from('playlist_user_song').insert([
          {
    ...
    case 'DELETE':
      if (!query.id) {
        ...
        const { error } = await supabase.from('playlist_user_song').delete().eq('id', query.id);
        if (error) {
          res.status(422).json({ error: error.message });
        }
