...
export default function Playlist() {
  const { data, error } = useSWR(`${process.env.NEXT_PUBLIC_API_ROUTE}/api/playlist`, fetcher);
  ...
  async function handleCreate() {
    ...
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_ROUTE}/api/playlist`, { name: name });
      if (res.status == 200) {
      ...
      mutate(`${process.env.NEXT_PUBLIC_API_ROUTE}/api/playlist`);
    }
  }
  ...
  async function handleEdit() {
    ...
    try {
      const res = await axios.put(`${process.env.NEXT_PUBLIC_API_ROUTE}/api/playlist`, editItem);
      if (res.status == 201) {
  ...
  async function handleDelete() {
    ...
    try {
      const res = await axios.delete(`${process.env.NEXT_PUBLIC_API_ROUTE}/api/playlist?id=${deleteItem.id}`);
      if (res.status == 200) {
