...
export default function Album() {
  ...
  const [editItem, setEditItem] = useState({ id: null, name: '', cover_url: '', artist_id: null });
  const [deleteItem, setDeleteItem] = useState({ id: null, name: '', artist_id: null });
  ...
  async function handleEdit() {
    ...
    try {
      ...
      if (res.status == 201) {
        ...
        setEditItem({ id: null, name: '', cover_url: '', artist_id: null });
        updateToast({ toastId, message: res.data.message, isError: false });
      }
    } catch (error) {
      console.error(error);
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        setDeleteItem({ id: null, name: '', artist_id: null });
        updateToast({ toastId, message: res.data.message, isError: false });
      }
    } catch (error) {
      console.error(error);
